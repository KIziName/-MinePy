...
class GamePlayer:
    ...
    def spawn(self):
        ...
        # --- Поднимаем игрока, если он застрял в блоке (например, в дереве) ---
        attempts = 200
        while self._check_collision() and attempts > 0:
            self.y -= 1
            attempts -= 1
        if attempts == 0:  # запасной вариант
...
class Game:
    def __init__(self):
        ...
        self.screen = pygame.display.set_mode((self.screen_width, self.screen_height))  # <-- флаги удалены
    ...
    def toggle_fullscreen(self):
        ...
        if self.is_fullscreen:
            self.screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)  # <-- флаги удалены
    ...
    def load_world(self):
        ...
        try:
            ...
            self.day_counter = data.get('day_counter', 0)
        ...
        if button == 1:
            ...
            if slot['type'] in placeable and slot['count'] > 0:
                if self.world.get_block(gx, gy) == BLOCK_AIR:
                    # Проверка, не перекрывает ли новый блок игрока (исправление пункта 2)
